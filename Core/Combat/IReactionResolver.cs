...
/// the framework's hurtbox from project-specific reaction outcome systems.
///
/// <para>
...
public interface IReactionResolver
{
    ...
    /// wrapper with the base <see cref="Jmodot.Implementation.Combat.Effects.DamageEffect"/> stripped (when an Exclusive reaction matched).
    /// </summary>
    /// <param name="attackerIdentity">The attacking spell's <see cref="Identity"/>, resolved via
    /// <c>IIdentifiable.GetIdentity()</c> on the attacker node hierarchy.</param>
    /// <param name="defenderIdentity">The defender's <see cref="Identity"/>, resolved similarly
    /// from the combatant's owner.</param>
    /// <param name="defenderActiveTags">All <see cref="CombatTag"/>s currently active on the defender
    /// (typically aggregated from the defender's <see cref="Jmodot.Implementation.Combat.StatusEffectComponent"/>). Empty list is
    /// fine; reactions that gate on status will simply not match.</param>
    /// <param name="attackerNode">The attacker node (typically <c>payload.Attacker</c>).</param>
    /// <param name="defenderNode">The defender node (typically <c>combatant.OwnerNode</c>).</param>
