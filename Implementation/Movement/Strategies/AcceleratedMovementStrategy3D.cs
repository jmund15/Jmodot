...
public partial class AcceleratedMovementStrategy3D : Resource, IMovementStrategy3D
{
        ...
        StatContext activeContext, float delta)
    {
        ...
        var targetVelocity = desiredDirection * stats.GetStatValue<float>(GlobalRegistry.DB.MaxSpeedAttr, activeContext);
        ...
        if (!desiredDirection.IsZeroApprox())
        {
            newVelocity = newVelocity.MoveToward(targetVelocity,
                stats.GetStatValue<float>(GlobalRegistry.DB.AccelerationAttr, activeContext) * delta);
        }
        else // If no input, apply friction.
        {
            ...
            newVelocity = newVelocity.MoveToward(Vector3.Zero,
                stats.GetStatValue<float>(GlobalRegistry.DB.FrictionAttr, activeContext) * delta);
        }
