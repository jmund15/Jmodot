...
public struct StatEffect : IRevertibleCombatEffect
{
    ...
    public CombatResult? Apply(ICombatant target, HitContext context)
    {
        ...
        {
            JmoLogger.Debug(this, $"[Reaction] StatEffect skipped — target '{target.GetUnderlyingNode().Name}' lacks attribute '{Attribute.AttributeName}'");
            return null;
        }
        Handle = handle;
    ...
    public ICombatEffect? GetRevertEffect(ICombatant target, HitContext context)
    {
        if (Handle == null)
        {
            JmoLogger.Debug(this, $"[Reaction] Can't get revert effect — stat effect wasn't applied on '{target.GetUnderlyingNode().Name}'.");
            return null;
        }
        if (!target.Blackboard.TryGet<StatController>(BBDataSig.Stats, out var stats))
        {
